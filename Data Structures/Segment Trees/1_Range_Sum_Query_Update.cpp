...
int rangeQuery(int* segmentTree, int start, int end, int treeNode, int left, int right) {
	...
	int mid = (start + end) / 2;
	int ansLeft = rangeQuery(segmentTree, start, mid, 2 * treeNode, left, right);
	int ansRight = rangeQuery(segmentTree, mid + 1, end, 2 * treeNode + 1, left, right);
...
int main()
{
	...
	int left, right;
	cin >> left >> right;
	cout << "Sum between the range is: " << rangeQuery(segmentTree, 0, n - 1, 1, left, right) << endl;
	return 0;
}